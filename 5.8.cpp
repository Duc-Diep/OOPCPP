...
class BenhNhan{
    protected:
        char name[15], quequan[15];
        int year;
    public:
        void nhap();
       	void xuat();
       	friend int Age(BenhAn list[],int i);
};
...
int Age(BenhAn list[],int i){
	return 2020 - list[i].year;
}
void sortByAge(BenhAn list[],int n){
	for(int i=0;i<n-1;i++){
		for(int j=i+1;j<n;j++){
			if (Age(list,i) < Age(list,j)){
				BenhAn temp = list[i];
				list[i] = list[j];
				list[j] = temp;
			}
...
void showAgeLess10(BenhAn list[], int n){
	...
	if (!check) cout<<"\tKhong co benh nhan nao"<<endl;
}
